...
const resolvers = {
  Query: {
    ...
    getPosts: async (_, { page = 1, limit = 10, searchTerm = "" }, context) => {
      // Assuming you have a context set up for error handling, or user authorization

      try {
        // Calculate the skip value for pagination
        const skip = (page - 1) * limit;

        // Build the search query if a searchTerm is provided
        // Ensure your Post schema has a text index for fields that you want to search
        let searchQuery = searchTerm.trim()
          ? { $text: { $search: searchTerm } }
          : {};

        // Find posts matching the search query with pagination and count the total documents
        ...
        // Map over the posts to handle likes and dislikes (if you have such logic)
        // As you mentioned, ensuring uniqueness and then populating likes and dislikes
        posts = await Promise.all(
          posts.map(async (post) => {
            const likes = await User.find({ _id: { $in: post.likes } });
            const dislikes = await User.find({ _id: { $in: post.dislikes } });
        ...
        // Return the paginated posts along with total number of posts
        return {
          posts: posts,
          total: total,
          page: page,
        ...
        // Error handling logic, could throw a custom GraphQL error or log the error
        console.error("Error retrieving posts:", error);
        throw new ApolloError("Error retrieving posts");
      }
    },
  ...
  Mutation: {
    ...
    changePassword: async (_, { oldPassword, newPassword }, { user }) => {
      ...
      const result = await changePasswordGraphQL(
        user._id,
        oldPassword,
        newPassword
      );
